...
def read_image_pil(image_uri, grayscale=True) -> Image.Image:
    if isinstance(image_uri, Image.Image):
        return image_uri.convert("L") if grayscale else image_uri
    with smart_open.open(image_uri, "rb") as image_file:
        image = Image.open(image_file)
        return image.convert("L") if grayscale else image



def read_image_pil_file(image_file, grayscale=True) -> Image:
...
def read_b64_image(b64_string, grayscale=True):
